def generate_measurement_report(measurements):
    ...
    face_width_px = face_proportions.get('face_width_px', 0)  # 얼굴 가로 길이 (픽셀 단위)
    ...
    report.append(f"\n눈썹 분석:")
    if face_width_px == 0 or left_eyebrow_length == 0 or right_eyebrow_length == 0:
        report.append("  → 데이터 부족: 얼굴 가로 길이 또는 눈썹 정보를 제공해주세요.")
    else:
        # 눈썹 평균 길이가 얼굴 가로 길이에서 차지하는 비율 계산 (%)
        eyebrow_ratio = (avg_eyebrow_length / face_width_px) * 100
        ...
        report.append(f"- 평균 눈썹 길이가 얼굴 가로 길이에서 차지하는 비율: {eyebrow_ratio:.1f}%")
        
        # 비율에 따른 분석 기준 설정
        if eyebrow_ratio > 45:
            report.append("  → 긴 눈썹")
        elif eyebrow_ratio > 25:
    ...
    if glabella_length == 0:
        ...
        if glabella_face_ratio > 25:
            report.append("  → 긴 미간")
        elif glabella_face_ratio > 15:
    ...
    if left_cheek_width_px == 0 or left_cheek_height_px == 0 or right_cheek_width_px == 0 or right_cheek_height_px == 0:
        ...
        if cheek_face_ratio > 25:
            report.append("  → 큰 볼")
        elif cheek_face_ratio > 15:
